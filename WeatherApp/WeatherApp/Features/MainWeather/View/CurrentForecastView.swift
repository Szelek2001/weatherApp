...
struct CurrentForecastView: View {
    ...
    var body: some View {
        VStack {
            ...
            Text(String(Int(viewModel.temperature ?? .zero)) + Units.celsius)
                .modifier(TitleModifiers())
            Spacer()
            (viewModel.currentWeather?.icon ?? Symbols.sunrise).font(.system(size: 100))
            Spacer()
            Text(Subtitles.from + String(viewModel.temperatureMin ?? .zero)
                 + Units.celsius + Subtitles.until + String(viewModel.temperatureMax ?? .zero)
                 + Units.celsius)
